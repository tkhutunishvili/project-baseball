Plotly.d3.json('/samples-win-loss-results-new', function (err, data) {
  ...
  for (var i = 1; i < data.length; i++) {
    var datum = data[i];
    var trace = getData(datum.year, datum.abbr);
    trace.text.push(datum.team_name);
    trace.id.push(datum.team_name);
    trace.x.push(datum.win);
  ...
  var teams = Object.keys(firstYear);

  // Create the main traces, one for each continent:
  var traces = [];
  for (i = 0; i < teams.length; i++) {
    var data = firstYear[teams[i]];
  ...
  var layout = {
    ...
    yaxis: {
      title: 'Team Performance (Win/Loss)',
      type: 'log'
    },
    hovermode: 'closest',
